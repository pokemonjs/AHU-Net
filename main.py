...
# # 不要忘记BatchNorm（数据预处理）模型收敛与否可能与之关系很大
# 居然仅仅是减去均值除以最大值就开始收敛了！！！
# 
# 必须除以最大值将输入约束到[-1,1]之间，感觉模型在这个区间内表现比较好，所以可以收敛

# In[ ]:


# get_ipython().system('unzip -qo seg_train.zip -d seg_train')
